...
## 8. 텍스트 표시 (줄바꿈·글씨 크기, #201)

- 모든 사용자 표시 한글 문구는 단어 단위로만 넘어가야 한다.
  `core/widgets/word_wrap.dart`의 `keepWordsTogether()`를 표시 직전에 적용하고
  (원문은 저장소에서 그대로 둔다, #177),
  신규 문구에도 의무 적용한다. 버튼·한 줄 라벨 등 줄바꿈이 없는 곳은 제외.
- 시스템 글씨 크기는 `app.dart`의 `MediaQuery.withClampedTextScaling`
  (최소 1.0·최대 1.2)으로 고정한다. Android 시스템 글씨가 그대로 들어오면
  고정 도트 레이아웃이 깨지고, iOS는 1.0을 유지해 기기별 편차가 생긴다 (#201).
  상한을 올리거나 풀 때는 고정 박스 화면(말씨·온보딩·잔디)을 먼저 점검한다.
- 본문처럼 길어질 수 있는 문구는 `Flexible` + `SingleChildScrollView`로 감싸
  좁은 화면·큰 글씨에서도 잘리지 않게 한다 (온보딩 `_PageBody` 참조).

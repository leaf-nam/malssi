...
- **배달 게이트** (#196, 2026-09-15, 구현됨):
  ...
  (수확 12시간 쿨다운 `Seed.cooldown`·`Seed.isCoolingDown` 폐지).
...
- **에셋 해상도** (고해상도 소스 교체, #173 — 2026-09-15, nearest-neighbor 4x 업스케일):
  씨앗 308px(77×4) · 성장 688px(172×4) · 열매 608px(152×4).
  1x 그리드 기준 외곽 1px에 검정 테두리를 둘러 아트 바깥으로만 확장
  (아트 픽셀 보존, 캔버스 각 +2px) — 가장자리 흰 fringe가 테두리 뒤로 가려진다.
  단 `grape_seed`는 원본 자체가 150px(75px의 정확한 2x가 아님)이라 608px(152×4) 유지 —
  75px로 내리면 픽셀 손실이 생겨 목표(씨앗 300px+)는 충족하므로 통일로 간주.

...
- **동작 플로우 (목표)**:
  ...
  6. 개발 모드 한정 임시 버튼 (`kDebugMode` — `디버그: +1단계` 1단계 진행,
     `디버그: 열매 만들기` 즉시 완성·수확, `디버그: +1일` 날짜 이동, #69, #95).
...
- **그리드 표시**: 화면 너비에 53주 전체가 최소 칸(22px) 이상으로 들어가면
  ...
  첫 진입에는 오늘이 가운데 오도록 이동한다 (스크롤 모드만, #98).
  오늘이 마지막 주라 뒤에 미래 빈칸(탭 불가)을 붙여 가운데 정렬한다.
...
- **테마 연출**: 가장 많이 모은 색깔(`topTheme`)의 열매 12개가 배경에 대각선으로
  비처럼 내린다 (#89, `FruitRain` — 14초 주기·불투명도 라이트 0.16/다크 0.22).
  모은 열매가 없으면 배경 연출 없음.
